...
$(document).ready(function () {
    // Mbyll fillimisht të gjitha .rx-child
    $('.rx-child').show();

    // Kur klikon .rx-parent, shfaq ose mbyll vetëm një .rx-child
    $('.rx-parent').on('click', function () {
        // Merr elementin përkatës .rx-child
        var $thisChild = $(this).siblings('.rx-child');

        // Mbyll të gjitha .rx-child
        $('.rx-child').slideUp();

        // Largo klasën nga të gjitha .rx-parent
        $('.rx-parent').removeClass('rx-change');

        // Nëse ky .rx-child nuk është i hapur, hapet
        if (!$thisChild.is(':visible')) {
            $thisChild.slideDown();
            $(this).addClass('rx-change');
        }
    });

    // Klikimi në .category-heading për me shfaq menunë
